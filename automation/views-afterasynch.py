...
class TaskDetailView(View):
    def get(self, request, id):
        ...
        # Get base queryset for tasks based on user role
...
@require_POST
@csrf_exempt
async def update_business_status(request, business_id):
    """
    Update business status and handle move-to-app operations with improved error handling and type processing.
    """
    async def get_status_counts(old_status, new_status):
        return {
            'old_status_count': await Business.objects.filter(status=old_status).acount(),
            'new_status_count': await Business.objects.filter(status=new_status).acount()
        }

    async def validate_descriptions(business, new_status):
        missing = []
        if new_status in ['REVIEWED', 'IN_PRODUCTION']: 
            ...
            if new_status == 'IN_PRODUCTION':
                for field, name in [
                    ...
                    ('description_esp', 'Spanish description'),
    ...
    async def prepare_move_to_app_data(business, user_id):
        business_data = model_to_dict(business)
        
        # Format operating hours
        if operating_hours := business_data.get('operating_hours'):
            business_data['operating_hours'] = format_operating_hours(operating_hours)
        ...
        # Process categories
        main_category = await get_category_by_title(
            title=business.main_category,
            level=task_obj.level
        )
        if not main_category:
            raise ValueError(f"Main category not found: {business.main_category}")
            
        business_data["category_id"] = main_category.ls_id
        
        if business.tailored_category:
            if sub_category := await get_category_by_title(
        ...
        business_data["city_id"] = await Destination.objects.filter(
            name__iexact=business.city
        ).values_list('ls_id', flat=True).afirst()
        
        country_obj = await Country.objects.filter(
            name__iexact=business.country
        ).afirst()
        
        # Get approved images
        image_urls = await Image.objects.filter(
        ...
        return {
            **business_data,
            'country': model_to_dict(country_obj),
            'user': model_to_dict(await CustomUser.objects.aget(id=user_id)),
            'images_urls': list(image_urls)
        }

    try:
        # Main execution
        business = await Business.objects.select_related(
            'task__level'
        ).prefetch_related(
            'images'
        ).aget(id=business_id)
        ...
        # Process types
        all_types = []
        for types_str in [business.types, business.types_eng, business.types_esp, 
                         business.types_fr, business.types_uk]:
            if types_str:
                all_types.extend(t.strip() for t in types_str.split(','))

        processed_types = await process_scraped_types(all_types, business.main_category)
        for field, value in processed_types.items():
            setattr(business, field, value)

        # Validate descriptions
        if missing_descriptions := await validate_descriptions(business, new_status):
        ...
        # Update status
        if new_status not in dict(Business.STATUS_CHOICES):
            return JsonResponse({'status': 'error', 'message': 'Invalid status'}, status=400)
        ...
        try:
            if business.task:
                await update_task_status_signal(business.task, business)
        except Exception as e:
        ...
        if new_status == 'IN_PRODUCTION':
            try:
                result_data = await prepare_move_to_app_data(business, user_id)
                app_data = json.dumps(result_data, default=datetime_serializer)
                
                # First attempt with types
                try:
                    await RequestClient().request('move-to-app', app_data)
                    return JsonResponse({'status': 'success', 'message': 'Business successfully moved to LS'})
                ...
                # Second attempt without types
                    ...
                    try:
                        await RequestClient().request(
                            'move-to-app', 
                            json.dumps(result_data_without_types, default=datetime_serializer)
                        )
                        return JsonResponse({
                            'status': 'success-with-warning',
                            'message': 'Business moved to LS successfully, but without types',
                            'removed_types': removed_types,
                            'warning': f'Types removed to complete move: {removed_types}. Add manually in LS backend.'
                        })
                    except Exception as second_error:
                        raise Exception(f"{const.MOVE_TO_APP_FAILED_MESSAGE}{str(second_error)}")
                        
            except Exception as e:
                return JsonResponse({'status': 'error', 'message': str(e)}, status=400)

        # Return status update success
        counts = await get_status_counts(old_status, new_status)
        return JsonResponse({
            'status': 'success',
            'new_status': new_status,
            'old_status': old_status,
        ...
        logger.error(
            f"Update business status failed: {str(e)}", 
            exc_info=True,
