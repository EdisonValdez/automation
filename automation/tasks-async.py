...
from automation.tag_mapper import TagMappingService, is_english 
from .models import BusinessImage, Country, Destination, HourlyBusyness, PopularTimes, Review, ScrapingTask, Business, Category, OpeningHours, AdditionalInfo, Image, TagMapping
...
def process_scraping_task(self, task_id, form_data=None):
    """
    Process a scraping task with proper async handling
    """
    async def async_process_task():
        ...
        try:
            ...
            task = await ScrapingTask.objects.aget(id=task_id)
            ...
            if not queries and task.file:
                ...
                async with aiofiles.open(task.file.name, 'r') as file:
            ...
            for index, query_data in enumerate(queries, start=1):
                ...
                while True:
                    ...
                    results = await process_query(query_data)
                    if results is None:
                        break
                    ...
                    await save_results(task, results, query)
                    ...
                    for result_index, local_result in enumerate(local_results, start=1):
                        try:
                            ...
                            async with transaction.atomic():
                                # Await the save_business coroutine
                                business = await save_business(task, local_result, query, form_data=form_data)
                                
                                if business and hasattr(business, 'id'):  # Check if business exists and has id
                                    logger.info(f"Downloading images for business {business.id}")
                                    await download_images(business, local_result, image_count=image_count)
                                else:
                    ...
                    if next_page_token:
                        ...
                        page_num += 1
                        await asyncio.sleep(random.uniform(2, 20))  # Async delay
                    else:
    ...
    return asyncio.run(async_process_task())
...
async def translate_business_types(business, languages, tag_ls_id=None):
    """
    Handles the translation of business tags and types using TagMapping service.
    Supports Spanish, UK English, American English, and French translations.
    """
    try:
        mapper = TagMappingService()
        original_types = business.types or []
        fields_to_update = []
        
        if not original_types:
            logger.warning(f"No types to translate for business {business.id}")
            return False

        # Validate languages input
        valid_languages = {"spanish", "eng", "uk", "fr"}
        invalid_languages = set(languages) - valid_languages
        if invalid_languages:
            logger.warning(f"Invalid languages detected: {invalid_languages}")
            languages = [lang for lang in languages if lang in valid_languages]

        # Helper function for translations
        async def translate_tags(lang_key, attr, lang_name):
            if not getattr(business, attr):
                translated_types = []
                for type_tag in original_types:
                    try:
                        translated_tag = await mapper.get_or_create_mapping(
                            type_tag, target_lang=lang_key, tag_ls_id=tag_ls_id
                        )
                        if translated_tag:
                            translated_types.append(translated_tag)
                    except Exception as tag_error:
                        logger.error(
                            f"Error translating tag '{type_tag}' to {lang_name} for business {business.id}: {str(tag_error)}",
                            exc_info=True,
                        )
                
                if translated_types:
                    setattr(business, attr, translated_types)
                    fields_to_update.append(attr)
                    logger.info(
                        f"{lang_name.capitalize()} types generated for business {business.id}: {translated_types}"
                    )
                else:
                    logger.info(
                        f"Types NOT generated for business {business.id}"
                    )

        # Perform translations for requested languages
        for lang in languages:
            if lang == "spanish":
                await translate_tags("spanish", "types_esp", "spanish")
            elif lang == "eng":
                if not business.types_eng:
                    if all(is_english(type_tag) for type_tag in original_types):
                        business.types_eng = original_types
                        logger.info(f"Original types used for American English: {original_types}")
                    else:
                        await translate_tags("eng", "types_eng", "american english")
            elif lang == "uk":
                await translate_tags("uk", "types_uk", "british english")
            elif lang == "fr":
                await translate_tags("french", "types_fr", "french")

        # Verify translations before saving
        translations_to_verify = {
            'spanish': business.types_esp if 'types_esp' in fields_to_update else None,
            'eng': business.types_eng if 'types_eng' in fields_to_update else None,
            'uk': business.types_uk if 'types_uk' in fields_to_update else None,
            'fr': business.types_fr if 'types_fr' in fields_to_update else None
        }

        # Remove empty translations
        fields_to_update = [
            field for field in fields_to_update 
            if getattr(business, field, None) and len(getattr(business, field)) > 0
        ]

        if fields_to_update:
            await business.asave(update_fields=fields_to_update)
            logger.info(
                f"Updated business {business.id} types in languages: {', '.join(languages)}\n"
                f"Fields updated: {fields_to_update}\n"
                f"Translations: {translations_to_verify}"
            )
            return True
        
        logger.warning(
            f"No fields were updated for business {business.id}. "
        ...
        logger.error(
            f"Error in translate_business_types for business {business.id}: {str(e)}", 
            exc_info=True
        )
        return False
...
@transaction.atomic
async def save_business(task, local_result, query, form_data=None):
    ...
    try: 
        ...
        if scraped_types:
            # Process types with translations
            processed_types = await process_scraped_types(
                scraped_types, 
                main_category=business_data.get('main_category')
            )
            logger.info(f"Processed business types: {processed_types}")
            
            # Update business_data with all type fields
            business_data.update({
                'types': processed_types['types'],
                'types_eng': processed_types['types_eng'],
                'types_esp': processed_types['types_esp'],
                'types_uk': processed_types['types_uk'],
                'types_fr': processed_types['types_fr']
            })
        ...
        business, created = await Business.objects.aget_or_create(
