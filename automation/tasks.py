...
def process_scraping_task(task_id, form_data=None):
    ...
    try:
        ...
        obj = s3_client.get_object(Bucket="businesses", Key=task.file.name)
            ...
            while start_offset < 100:
                ...
                for result_index, local_result in enumerate(local_results, start=1):
                    try:
                        ...
                        if business:
                            logger.info(f"Business {business.id} saved successfully.")
                        else:
                            logger.warning(f"Business '{local_result.get('title', 'Unknown')}' skipped due to errors.")
...
# Retry mechanism for image downloads (using backoff library)
@backoff.on_exception(backoff.expo, requests.exceptions.RequestException, max_tries=5)
def download_image_with_retry(image_url):
    response = requests.get(image_url, timeout=10)
    response.raise_for_status()  # Raise exception for HTTP errors
    return response.content
 

def upload_image_to_s3(client, file_path, file_content, content_type='image/jpeg'):
    try:
        client.put_object(
        ...
        logger.info(f"Uploaded {file_path} to S3 successfully.")
    except Exception as e:
        logger.error(f"Error uploading {file_path} to S3: {str(e)}")
        return None

    # Construct the public URL of the uploaded file
    return f"{settings.AWS_S3_ENDPOINT_URL}/{file_path}"
    # Or if you need the CDN URL, adjust accordingly
...
def download_images(business, local_result):
    ...
    try:
        ...
        for i, photo in enumerate(photos_results.get('photos', [])):
            ...
            if image_url:
                try:
                    ...
                    # Optionally process the image (e.g., crop to aspect ratio)
                    aspect_ratio = 3 / 2
                    ...
                    # Upload image to S3
                    media_url = upload_image_to_s3(s3_client, file_path, buffer.getvalue())
                    ...
                    if media_url:
                        ...
                        logger.error(f"Failed to upload image {i} for business {business.id}")

                except Exception as e:
                    logger.error(f"Error processing image {i} for business {business.id}: {str(e)}", exc_info=True)

            time.sleep(1)  # Avoid overloading the API server
        ...
        if first_image:
            business.main_image = first_image.image_url
            business.save()
...
def save_results(task, results, query):
    try:
        ...
        buffer = BytesIO(json_content.encode('utf-8'))  # Convert JSON to byte stream

        # Upload JSON to S3
        upload_image_to_s3(s3_client, file_path, buffer, content_type='application/json')
...
def save_business(task, local_result, query, form_data=None):
    ...
    try:
        ...
        lock_acquired = acquire_lock(task.id)
        if not lock_acquired:
            logger.error(f"Task {task.id} is already being processed, skipping.")
            return None
        ...
        with transaction.atomic():
            ...
            if form_data:
                ...
                if form_country_id and form_country_name:
                    try:
                        ...
                        business_data['country'] = country_obj.name  # Override country
                        business_data['country_obj'] = country_obj
                        logger.info(f"Country overridden by form data: {country_obj.name}")
                    except Country.DoesNotExist:
                        logger.error(f"Country with ID {form_country_id} and name '{form_country_name}' does not exist.")
                        business_data['country_obj'] = None
                else:
                    business_data['country_obj'] = None

                # Validate and assign destination
                form_destination_id = form_data.get('destination_id')
                form_destination_name = form_data.get('destination_name')

                if form_destination_id and form_destination_name and business_data['country_obj']:
                    try:
                        destination_obj = Destination.objects.get(
                            id=form_destination_id,
                            name__iexact=form_destination_name,
                            country=business_data['country_obj']
                        )
                        business_data['destination'] = destination_obj
                        logger.info(f"Destination overridden by form data: {destination_obj.name}")
                    except Destination.DoesNotExist:
                        logger.error(f"Destination with ID {form_destination_id} and name '{form_destination_name}' does not exist.")
                        business_data['destination'] = None
                else:
                    business_data['destination'] = None
                ...
                if business_data['country']:
                    ...
                    elif business_data['state']:
                        destination_name = business_data['state']
                    else:
                    ...
                    try:
                        ...
                        logger.warning(f"Destination '{destination_name}' does not exist.")
                        business_data['destination'] = None
                else:
                    logger.error("Country information is missing, cannot assign Destination.")
                    business_data['country_obj'] = None
                    business_data['destination'] = None
            ...
            additional_info_list = [
                ...
                for key, value in local_result.get('additional_info', {}).items()
            ]
            if additional_info_list:
                AdditionalInfo.objects.bulk_create(additional_info_list, ignore_conflicts=True)
                logger.info(f"Additional data saved for business {business.id}")

        # Image downloading is done outside the transaction to prevent rollback due to image errors
        download_images(business, local_result)
        logger.info(f"Images downloaded for business {business.id}")

        return business

    except Exception as e:
        logger.error(f"Error saving business data for task {task.id}: {str(e)}", exc_info=True)
        # Do not re-raise the exception to allow processing of other businesses
        return None
    
    finally:
        # Release the lock after the task completes
        if lock_acquired:
